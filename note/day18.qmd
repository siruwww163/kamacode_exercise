...
**BST 的中序遍历结果是一个非递减序列，也就是说，相同的值一定是连在一起出现的。**
...
class Solution:
    ...
    def searchBST(self, cur):
        ...
        if self.pre is None:  # 第一个节点
            self.count = 1
        elif self.pre.val == cur.val:  # 与前一个节点数值相同，这里比的是val，要记得写val
            self.count += 1
        else:  # 与前一个节点数值不同
            self.count = 1
...
>
> ![](/image/236example1.png){width=50%}
>
> ![](/image/236example2.png){width=50%}
